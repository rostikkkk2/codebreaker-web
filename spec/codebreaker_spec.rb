...
RSpec.describe Codebreaker do
  ...
  context 'when welcome' do
    describe 'home page' do
      let(:response) { get Codebreaker::URLS[:home] }

      it 'is welcome home page' do
        expect(response).to be_ok
        expect(response.body).to include I18n.t(:player_name)
      end
    end
  end

  describe 'when show rules' do
    let(:response) { get Codebreaker::URLS[:rules] }
  ...
  describe 'when show statistics' do
    let(:response) { get Codebreaker::URLS[:statistics] }
  ...
  describe 'when user registrate' do
    let(:response) do
      post Codebreaker::URLS[:registration], player_name: user_name, level: test_difficulty
    end
  ...
  context 'when show hint' do
    let(:response) { post Codebreaker::URLS[:show_hint] }
    let(:response_registration) do
      post Codebreaker::URLS[:registration],
    ...
    describe 'show hint' do
      ...
      it 'show hint' do
        expect(response).to be_redirect
        expect(last_request.session[:hints_left]).to eq 1
      end
    end
  end
  ...
  describe 'when user win' do
    ...
    let(:response) { post Codebreaker::URLS[:play_game], guess_code: valid_code }
  ...
  describe 'when user lose' do
    ...
    let(:response) { post Codebreaker::URLS[:play_game], guess_code: invalid_code }
